...
def de_noise(X_i, image_height=96, image_width=96):
    """Filter out noise using median filtering."""
    h, w = image_height, image_width

    def de_noise_x_i(x_i):
...
class ImageCompressor:
    ...
    def get_2D_channel_matrices(self, images):
    ...
    def get_mean_image(self, X_i):
        return np.transpose(np.average(np.transpose(X_i), axis=0))

    def get_principals(self, X_i):
    ...
    def train(self, train_images, k=15) -> None:
        ...
        for X_i, color_str in zip(X_dict.values(), X_dict.keys()):
            # show_image(X_i.T[0].reshape((96, 96)), title=f"{color_str} colour channel before removing noise")
            if self.remove_noise:
                ...
                # show_image(X_i.T[0].reshape((96, 96)), title=f"{color_str} colour channel after removing noise.")
            X_i_mean = self.get_mean_image(X_i)
            X_i_0 = self.de_mean(X_i, X_i_mean)
            # show_image(X_i_0.T[0].reshape((96, 96)), title=f"{color_str} de-meaned")
        ...
        self.mean_image = np.array(mean_image).astype(np.float16)
        ...
        self.codebook = np.array(
            [
                np.hstack(
                    (
                        np.array([self.mean_image[i]]).T,
                        self.principal_directions[i],
                    )
                )
                for i in range(3)
            ]
...
class ImageReconstructor:
    ...
    def __init__(self, codebook, remove_noise=False):
